...
module.exports = {
	...
	getType: function (uid, callback) {
		var sql = "select type from users where uid=?";
	...
	validate: function (email, callback) {
		var sql = "select password, uid, type from users where email = ?";
		db.getResult(sql, [email], function (result) {
			callback(result);
		});
	},
	...
	updateSettingsCc: function (user, callback) {
		var sql = "update users set ccard=?, type=? where uid=?";
		console.log(user);
		db.execute(sql, [user.ccard, user.type, user.uid], function (status) {
			callback(status);
		});
	},
